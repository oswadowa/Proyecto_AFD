...
namespace Proyecto_AFD_Fase_II
{
    public partial class Form1 : Form
    {
        ...
        {
            string FullText = AutomataFN.Text;
            string Texto = CadenaFN.Text;
            int Líneas = Regex.Matches(FullText, Environment.NewLine).Count;
            string[] Jumps = new string[Líneas + 1];
            Jumps = FullText.Split('\n');
            ...
            string Vef = "";
            bool ConfFinal = false;
            string[] EstadosGuardados = new string[20];
            int[] PosLetras = new int[20];
            int PosGuardado = 0;
            int NRepeticion = 0;
            int[,] NextBif = new int[20, 20];
            bool passed = false;
            int GuardadoPassed = 0;
            int[] TextPassed = new int[10];
            ...
            for (int i = 0; i < Texto.Length; i++)
            {
                ...
                if (i == 0) //Si viene de entrada, utilizar siguiente comandos//
                {
                    for (int j = 0; j < CantIniciales; j++)
                    {
                        if (currentCharacter == Escribir[posIniciales[j]])
                        {
                            ...
                            if (NRepeticion != 0)
                            {
                                PosLetras[PosGuardado] = j;
                                EstadosGuardados[PosGuardado] = Vef;
                                for(int k = 0; k < 10; k++)
                                {
                                    NextBif[PosGuardado, k] = Saltos[j, k]; 
                                }
                                TextPassed[PosGuardado] = i;
                                PosGuardado++;
                            }
                            NRepeticion++;
                        }
                    ...
                    if(passed == false)
                    {
                        VerificacionLabelAFN.Text = "Rechazada";
                    }
                }
                ...
                else //Si se está evaluando una letra posterior a la primera se usa este código//
                {
                    for (int j = 0; j < nextPos.Length - 1; j++)
                    {
                        ...
                        if(passed = false && i > 0)
                        {
                            Vef = EstadosGuardados[GuardadoPassed];
                            for(int k = 0; k < 10; k++)
                            {
                                nextPos[k] = NextBif[GuardadoPassed, k];
                            }
                            i = TextPassed[GuardadoPassed];
                            GuardadoPassed++;
                        }
                    }
                }
            ...
            if (Vef == Texto && ConfFinal == true)
            {
                VerificacionLabelAFN.Text = "Aceptada";
            }
            else
            {
                VerificacionLabelAFN.Text = "Rechazada";
